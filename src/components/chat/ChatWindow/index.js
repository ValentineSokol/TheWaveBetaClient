...
import { fetchChatroomFromQuery, fetchChatroomById, sendMessage } from '../../../redux/actions/api/chat';
...
class ChatWindow extends Component {
  ...
  renderTypingMessage = () => {
    const wrapInHtml = (message) => (
      <div className="TypingMessage">
        {message}
        <Typed strings="..." showCursor={false} />
      </div>
    );
      ...
      <>
        ...
        <section className="OverlayInfo">
          <Heading size="3">{chatName}</Heading>
          {
                ...
                && (
                <span>
                  {this.state.companionOnline ? 'Online'
